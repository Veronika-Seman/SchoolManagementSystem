...
class WaitlistDAO(BaseDAO):
    ...
    def add_to_waitlist(self, student_id, course_id):
        ...
        VALUES (%s, %s, (SELECT IFNULL(MAX(position), 0) + 1 FROM Waitlist WHERE course_id = %s))
        """
        try:
            self.cursor.execute(query, (course_id, student_id, course_id))
            self.connection.commit()
            print(f"Student {student_id} added to the waitlist for course {course_id}.")
    ...
    def remove_from_waitlist(self, student_id, course_id):
        ...
        WHERE course_id = %s AND position > (SELECT position FROM Waitlist WHERE student_id = %s AND course_id = %s)
        """
        try:
            self.cursor.execute(update_query, (course_id, student_id, course_id))
            self.cursor.execute(query, (student_id, course_id))
            self.connection.commit()
